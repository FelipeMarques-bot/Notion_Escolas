...
def _select_escola_no_portal(page, escola: str, logger=None) -> bool:
    """Clica no link da escola na tela de seleção do portal SGE.

    Após o login, o portal exibe uma lista de escolas vinculadas ao professor.
    É necessário clicar na escola correta antes de acessar qualquer funcionalidade
    (Plano de Aulas, Notas, Frequência, etc.).

    Retorna True se o link foi encontrado e clicado, False se não havia links de
    escola visíveis (já estamos dentro de uma escola ou a lista não apareceu).
    """
    ...
    for scope in _iter_scopes(page):
        ...
        for i in range(total):
            link = links.nth(i)
            try:
                txt = (link.inner_text(timeout=200) or "").strip()
                if not txt:
                    continue
                if escola_norm in _normalize(txt):
                    link.click(timeout=ACTION_TIMEOUT_MS)
                    page.wait_for_timeout(1500)
                    _log(logger, f"Escola selecionada no portal: '{txt}'")
                    return True
            except Exception:  # noqa: BLE001
                continue
    return False
...
def _open_plano_aulas_for_context(page, contexto: ContextoPlano, logger=None) -> bool:
    # Passo 0: clicar na escola correta na tela de seleção do portal.
    _select_escola_no_portal(page, contexto.escola, logger=logger)
